...
*/
  ...
  for(int i=0;i<Nmax;i+=Nevents)
  {
    Nevents=0;
    usleep(1000000*dt);
    gettimeofday(&tStop,NULL);
    cout<<i<<endl;
    int nMax = readRate*get_ms(&tStart,&tStop)/1000.-i;
    cout<<"Time from start : "<<get_ms(&tStart,&tStop)/1000.<<"s"<<endl;
    cout<<"Rate from start : "<<i*1./get_ms(&tStart,&tStop)*1000.<<"Hz"<<endl;
    cout<<"Will read max "<<nMax<<"events."<<endl;
    int nToRead = myTDC->GetNumberOfEvents();
    cout<<nToRead<<" events in FIFO"<<endl;
    if (nToRead > nMax) nToRead=nMax;
    
    // Trigger throttling :
    unsigned int status = myTDC->GetStatusWord();
    if (myTDC->IsFull(status) || myTDC->IsAlmostFull(status)){
        if (triggerOn){
            t->changeChannel(0);
            triggerOn=false;
        }
    ...
    for (int j = 0; j< nToRead; j++){
        ...
        if (e.errorCode !=0 && e.errorCode!=4) cout<<"ERROR num "<<e.errorCode<<endl;
        else Nevents++;
    }
    ...
    for (unsigned int k=0; k<ev.size(); k++){
        ...
        if (e.errorCode !=0){
            ...
            for (int l=0; l<e.hits.size(); l++){
             //   cout<<e.hits.at(l).channel<<"-"<<e.hits.at(l).time<<endl;
            }
        }
